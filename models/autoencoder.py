...
class VisualTactileAutoencoder:
    def __init__(self, z_dim: int, modality="concat", lr=1e-3, optimizer=torch.optim.Adam, device="cuda"):
        self.z_dim = z_dim
    ...
    def report_images(self, front, tactile):
        ...
        input_image = np.transpose(x.cpu().detach().numpy(), (1, 2, 0))
        recon_image = np.transpose(recon.cpu().detach().numpy(), (1, 2, 0))
